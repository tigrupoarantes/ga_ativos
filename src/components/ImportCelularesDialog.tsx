...
const ImportRowSchema = z.object({
  ...
  imei: z
    ...
    .refine((v) => v.length > 0, "IMEI é obrigatório"),
});

type ImportRow = z.infer<typeof ImportRowSchema>;

type ImportLineResult =
  | { status: "ok"; index: number; modelo: string; imei: string; patrimonio: string }
  | { status: "skip"; index: number; modelo: string; imei: string; reason: string }
  | { status: "error"; index: number; modelo?: string; imei?: string; reason: string };
...
function mapObjectToImportRow(obj: Record<string, unknown>): { modelo?: unknown; imei?: unknown } {
  const normalizedEntries = Object.fromEntries(
    Object.entries(obj).map(([k, v]) => [normalizeHeaderKey(String(k)), v])
  );

  const modelo =
    normalizedEntries.modelo ??
    normalizedEntries.model ??
    normalizedEntries.aparelho_modelo ??
    normalizedEntries.modelo_aparelho;

  const imei = normalizedEntries.imei;
...
export function ImportCelularesDialog() {
  ...
  const tipoCelular = useMemo(() => {
    return tipos.find(
      (t) => t.name?.toLowerCase().includes("celular") || t.category?.toLowerCase().includes("celular")
    );
  }, [tipos]);

  const canImport = !!tipoCelular && rows.length > 0 && !isParsing && !isImporting;
  ...
  const loadRowsFromFile = async (selected: File) => {
    ...
    try {
      ...
      objects.forEach((obj, idx) => {
        ...
        if (!res.success) {
          errors.push({
            ...
            imei: typeof mapped.imei === "string" ? mapped.imei : undefined,
            reason: res.error.issues.map((i) => i.message).join("; "),
          });
          return;
        }

        if (seenImei.has(res.data.imei)) {
          errors.push({
            status: "skip",
            index: idx + 2,
            modelo: res.data.modelo,
            imei: res.data.imei,
            reason: "IMEI duplicado no arquivo",
          });
          return;
        }

        seenImei.add(res.data.imei);
      ...
      if (parsed.length === 0) {
        toast.error("Nenhuma linha válida encontrada. Verifique as colunas 'modelo' e 'imei'.");
      } else {
  ...
  const fetchExistingImeis = async (imeis: string[]): Promise<Set<string>> => {
    ...
    for (let i = 0; i < imeis.length; i += chunkSize) {
      ...
      for (const item of data || []) {
        if (item.imei) existing.add(String(item.imei).replace(/\D/g, ""));
      }
    }
  ...
  const handleImport = async () => {
    if (!tipoCelular) {
      toast.error('Tipo "Celular" não encontrado. Cadastre em Tipos de Ativos e tente novamente.');
      return;
    }
    ...
    try {
      ...
      const existingImeis = await fetchExistingImeis(imeis);
      ...
      const lineResults = await asyncPool(5, rowsToCreate, async ({ r, idx }) => {
        ...
        if (existingImeis.has(r.imei)) {
          return {
            status: "skip" as const,
            index: lineNumber,
            modelo: r.modelo,
            imei: r.imei,
            reason: "IMEI já existe no sistema",
          };
        }

        const nome = `Celular ${r.modelo}`.trim();

        try {
          const { data, error } = await supabase.rpc("create_asset_with_patrimonio", {
            p_tipo_id: tipoCelular.id,
            p_nome: nome,
            p_marca: null,
            p_modelo: r.modelo,
            p_numero_serie: null,
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Importar Ativos — Celulares</DialogTitle>
          <DialogDescription>
            Importação em massa para o tipo <strong>Celular</strong>. Campos obrigatórios: <strong>modelo</strong> e <strong>imei</strong>.
          </DialogDescription>
        </DialogHeader>
        ...
        {!tipoCelular && (
          <div className="rounded-md border p-3 text-sm">
            ...
            <div className="mt-2">
              Tipo "Celular" não encontrado em Tipos de Ativos. Cadastre/ative um tipo com nome ou categoria contendo "celular".
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground">
              Cabeçalhos esperados: <code>modelo</code> e <code>imei</code> (primeira linha).
            </p>
            <Button variant="link" asChild className="px-0 h-auto text-xs">
              <a href="/templates/import-celulares-template.csv" download>
                Baixar template (CSV)
              </a>
            </Button>
          </div>
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between">
                        <div>
                          ...
                          <span className="ml-2">Linha {r.index}: {r.modelo} — {r.imei}</span>
                        </div>
                        <span className="font-mono text-xs text-muted-foreground">{r.patrimonio}</span>
                      </div>
                    );
                  }

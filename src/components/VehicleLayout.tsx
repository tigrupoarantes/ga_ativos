...
import { Car, List, AlertTriangle, History, DollarSign } from "lucide-react";
...
const subNavItems = [
  ...
  { icon: DollarSign, label: "Custos", shortLabel: "Custos", path: "/veiculos/custos" },
];
          ...
          <nav className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 -mb-1 scrollbar-hide">
            {subNavItems.map((item, index) => {
              ...
              const isActive = location.pathname === item.path ||
                (item.path !== "/veiculos" && location.pathname.startsWith(item.path));

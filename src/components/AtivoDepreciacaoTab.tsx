...
import { useDepreciationData, useRecalculateDepreciation, useDepreciationHistory } from "@/hooks/useDepreciation";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";

interface AtivoDepreciacaoTabProps {
  assetId: string;
}
...
export function AtivoDepreciacaoTab({ assetId }: AtivoDepreciacaoTabProps) {
  const { data, isLoading } = useDepreciationData(assetId);
  const { data: history, isLoading: loadingHistory } = useDepreciationHistory(assetId);
  const recalculate = useRecalculateDepreciation();
      ...
      <div className="space-y-4">
        <div className="text-center py-8">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            {!data.valor_aquisicao || data.valor_aquisicao <= 0
              ? "Pendente de dados financeiros — informe o valor de aquisição para habilitar."
              : !data.data_aquisicao
              ? "Pendente de data de aquisição."
              : "Este ativo não está configurado para depreciação."}
          </p>
        </div>
        <div className="flex justify-center">
          <Button
            variant="outline"
            size="sm"
            onClick={() => recalculate.mutate(assetId)}
  ...
  const usefulLife = data.tipo?.useful_life_months || 60;
  const monthsElapsed = data.monthly_depreciation && data.accumulated_depreciation && data.monthly_depreciation > 0
    ? Math.round(data.accumulated_depreciation / data.monthly_depreciation)
    : 0;
  ...
  const percentDepreciated = data.depreciable_value && data.depreciable_value > 0
    ? Math.min(100, Math.round((data.accumulated_depreciation || 0) / data.depreciable_value * 100))
    : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {isFullyDepreciated && (
            <Badge className="bg-orange-500/10 text-orange-600">Totalmente depreciado</Badge>
          )}
          {isStopped && !isFullyDepreciated && (
            <Badge className="bg-red-500/10 text-red-600">Depreciação parada</Badge>
          )}
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="shadow-none">
          <CardContent className="pt-4 pb-3 px-4">
            ...
            <p className="text-lg font-semibold">{formatCurrency(data.valor_aquisicao)}</p>
          </CardContent>
        </Card>
        <Card className="shadow-none">
          <CardContent className="pt-4 pb-3 px-4">
            <p className="text-xs text-muted-foreground">Valor Residual ({formatPercent(data.residual_percent)})</p>
            <p className="text-lg font-semibold">{formatCurrency(data.residual_value)}</p>
          </CardContent>
        </Card>
        <Card className="shadow-none">
          <CardContent className="pt-4 pb-3 px-4">
            <p className="text-xs text-muted-foreground">Valor Depreciável</p>
            <p className="text-lg font-semibold">{formatCurrency(data.depreciable_value)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="shadow-none">
          <CardContent className="pt-4 pb-3 px-4">
            ...
            <p className="text-lg font-semibold text-orange-600">{formatCurrency(data.monthly_depreciation)}</p>
          </CardContent>
        </Card>
        <Card className="shadow-none">
          <CardContent className="pt-4 pb-3 px-4">
            <p className="text-xs text-muted-foreground">Depreciação Acumulada</p>
            <p className="text-lg font-semibold text-red-600">{formatCurrency(data.accumulated_depreciation)}</p>
          </CardContent>
        </Card>
        <Card className="shadow-none border-2 border-primary/20">
          <CardContent className="pt-4 pb-3 px-4">
            <p className="text-xs text-muted-foreground">Valor Contábil Líquido</p>
            <p className="text-lg font-bold text-primary">{formatCurrency(data.valor_atual)}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="w-full bg-muted rounded-full h-2.5">
          <div
            className={`h-2.5 rounded-full transition-all ${
              isFullyDepreciated ? "bg-orange-500" : "bg-primary"
            }`}
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm border rounded-lg p-4">
        <div className="flex justify-between">
          ...
          <span>{formatDate(data.data_aquisicao)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Início depreciação</span>
          <span>{formatDate(data.depreciation_start_date || data.entry_into_operation_date || data.data_aquisicao)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Vida útil</span>
          <span>{usefulLife} meses</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Último cálculo</span>
          <span>{formatDate(data.depreciation_last_calculated_at)}</span>
        </div>
        {data.depreciation_override && (
          <div className="col-span-2 flex justify-between">
            <span className="text-muted-foreground">Override</span>
            <Badge variant="outline" className="text-xs">
              {data.depreciation_override_reason || "Parâmetros customizados"}
            </Badge>
          </div>
        )}
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {history.slice(0, 10).map((h) => (
                  <TableRow key={h.id}>
                    <TableCell className="text-xs">{formatDate(h.calculated_at)}</TableCell>
                    <TableCell className="text-xs">{h.months_elapsed}</TableCell>
                    <TableCell className="text-xs">{formatCurrency(h.accumulated_depreciation)}</TableCell>
                    <TableCell className="text-xs font-medium">{formatCurrency(h.book_value)}</TableCell>

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Cliente para o banco DESTINO (GA Pagamentos)
    const gaPagamentosUrl = Deno.env.get("GAPAGAMENTOS_SUPABASE_URL");
    const gaPagamentosKey = Deno.env.get("GAPAGAMENTOS_SUPABASE_SERVICE_KEY");

    if (!gaPagamentosUrl || !gaPagamentosKey) {
      return new Response(
        JSON.stringify({ error: 'Credenciais do GA Pagamentos não configuradas' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const targetSupabase = createClient(gaPagamentosUrl, gaPagamentosKey);
      ...
      async start(controller) {
        ...
        try {
          ...
          if (syncType === 'empresas' || syncType === 'all') {
            console.log('Iniciando sincronização de empresas -> companies (GA Pagamentos)...');
          ...
          // ========== SYNC FUNCIONARIOS -> EXTERNAL_EMPLOYEES ==========
          if (syncType === 'funcionarios' || syncType === 'all') {
            console.log('Iniciando sincronização de funcionários (GA Pagamentos)...');
            ...
            if (funcError) {
              ...
              for (let i = 0; i < funcionariosOrigem.length; i++) {
                ...
                try {
                  ...
                  // Mapeamento completo para external_employees do GA Pagamentos
                  // Inclui campos adicionais: CNH completa, vendedor e WhatsApp
                  const employeeData = {
                    external_id: func.id,
                    ...
                    // Condutor e CNH completa
                    is_condutor: func.is_condutor ?? false,
                    cnh_numero: func.cnh_numero || null,
                    cnh_categoria: func.cnh_categoria || null,
                    cnh_validade: func.cnh_validade || null,
                    // Vendedor (campos novos)
                    is_vendedor: func.is_vendedor ?? false,
                    codigo_vendedor: func.codigo_vendedor || null,
                    // WhatsApp
                    whatsapp_phone_e164: func.whatsapp_phone_e164 || null,
          ...
          console.log('Sincronização GA Pagamentos concluída:', result);
